...
public class WifiAdmin {
    ...
    public void forget(String SSID) {
        ...
        for (WifiConfiguration existingConfig : existingConfigs) {
            ...
            if (existingConfig.SSID.toLowerCase().contains(SSID.toLowerCase())/*&& existingConfig.preSharedKey.equals("\"" + password + "\"")*/) {
                mWifiManager.setWifiEnabled(false);
              //  mWifiManager.disableNetwork(existingConfig.networkId);
                mWifiManager.removeNetwork(existingConfig.networkId);
                mWifiManager.saveConfiguration();
                System.out.println("leo forget : " + SSID);
            }
        }
    ...
    public void clearAll(String SSID) {
        List<WifiConfiguration> existingConfigs = mWifiManager.getConfiguredNetworks();
        //按照networkId从大到小排序
        // Collections.sort(existingConfigs, new ComparatorConfig());
        for (WifiConfiguration existingConfig : existingConfigs) {
            ...
            if (existingConfig.SSID.equals("\"" + SSID + "\"") /*&& existingConfig.preSharedKey.equals("\"" + password + "\"")*/) {
                ...
                mWifiManager.removeNetwork(existingConfig.networkId);
                mWifiManager.saveConfiguration();
                System.out.println("leo clearAll : " + SSID);
            }
        }
    }
  ...
  /*  public ArrayList<ScanResult> getWifiList24G() {
    ...
    public void removeExitConfig(String ssid) {
        ...
        if (exsits != null) {//如果已经连接过则删除
           // mWifiManager.disconnect();
            boolean b = mWifiManager.removeNetwork(exsits.networkId);
            mWifiManager.saveConfiguration();

            System.out.println("leo removeExitConfig : ssid = " + ssid + " reason = " + b);
        }

    }


    public void removeConfig(WifiConfiguration ssid) {
  /*      WifiConfiguration exsits = isExsits(ssid);
        if (exsits != null) {//如果已经连接过则删除*/
            //mWifiManager.disconnect();
            boolean b = mWifiManager.removeNetwork(ssid.networkId);
            mWifiManager.saveConfiguration();
