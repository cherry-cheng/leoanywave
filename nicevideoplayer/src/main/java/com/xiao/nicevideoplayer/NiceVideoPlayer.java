...
public class NiceVideoPlayer extends FrameLayout
    ...
    @Override
    public void restart(long skipToPosition) {
        ...
        if (mCurrentState == STATE_PAUSED) {
            ...
            mMediaPlayer.reset();
            openMediaPlayer(skipToPosition);
        } else {
            LogUtil.d("NiceVideoPlayer在mCurrentState == " + mCurrentState + "时不能调用restart()方法.");
        }
    }
    ...
    private void openMediaPlayer(long skipToPosition) {
        // 屏幕常亮
        ...
        try {
            ...
            if (skipToPosition!=0) {
                mMediaPlayer.seekTo(skipToPosition);
            }
